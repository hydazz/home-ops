...
function main() {
    ...
    local base patch type nvidia_gpu_present result
    ...
    # Check if NVIDIA GPU is present based on node labels
    if nvidia_gpu_present=$(yq --exit-status 'select(documentIndex == 0) | .machine.nodeLabels["nvidia.com/gpu.present"] // "false"' "${MACHINEPATCH}" 2>/dev/null); then
        : # nvidia_gpu_present is set
    else
        nvidia_gpu_present="false"
    fi

    # Render the base machine configurations
    if ! base=$(minijinja-cli --define "machinetype=${type}" --define "nvidia_gpu_present=${nvidia_gpu_present}" "${MACHINEBASE}" | op inject) || [[ -z "${base}" ]]; then
    ...
    if ! patch=$(minijinja-cli --define "machinetype=${type}" --define "nvidia_gpu_present=${nvidia_gpu_present}" "${MACHINEPATCH}" | op inject) || [[ -z "${patch}" ]]; then
